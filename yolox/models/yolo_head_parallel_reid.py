...
class YOLOXHead_ParaReID(nn.Module):
    def __init__(
        self,
        num_classes,
        width=1.0,
        ...
        self.nID = nID                                              # ReID. total ids in dataset
        self.emb_scale = math.sqrt(2) * math.log(self.nID - 1)      # ReID. embedding scale
        ...
        self.reid_classifier = nn.Linear(self.emb_dim, self.nID)        # ReID classifier, only used when training
        self.s_det = nn.Parameter(-1.85 * torch.ones(1))                # For Uncertainty loss
        self.s_id = nn.Parameter(-1.05 * torch.ones(1))                 # For Uncertainty loss
        # self.s_det = nn.Parameter(3 * torch.ones(1))                # For Uncertainty loss
        # self.s_id = nn.Parameter(2 * torch.ones(1))                 # For Uncertainty loss
        self.settings = {}
        ...
        for i in range(len(in_channels)):       # iteration over levels of output features
            ...
            # reid head: reid_convs (2 * 3x3 Conv) + reid_preds (1 * 1x1 Conv)
        ...
        self.IDLoss = nn.CrossEntropyLoss(ignore_index=-1)      # ReID loss. CrossEntropyLoss, ignore -1 ids (e.g. no id information)
    ...
    def forward(self, xin, labels=None, imgs=None, track_ids=None, train_epoch=0):     # xin: 256/8, 512/16, 1024/32
        outputs = []
        origin_preds = []
        x_shifts = []
        y_shifts = []
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                ...
                # reid output. obj_output, cls_output use sigmoid --> obj_output, cls_output use sigmoid, reid_output
                output = torch.cat(
        ...
        if self.training:
            return self.get_losses(
                ...
                expanded_strides,
                labels,
                track_ids, # fix track_id bug when FP16
                torch.cat(outputs, 1),
                origin_preds,
                dtype=xin[0].dtype,
            )
        else:
            # no need to modify for reid
    ...
    def decode_outputs(self, outputs, dtype):       # outputs: [1, 23625, 6], 23625 = 1440/32 * 800/32 * (1 + 2^2 + 2^4)
        grids = []
        strides = []
        ...
        grids = torch.cat(grids, dim=1).type(dtype)         # [1, (H/8 * W/8) + (H/16 * W/16) + (H/32 * W/32), 2]
        strides = torch.cat(strides, dim=1).type(dtype)     # [1, (H/8 * W/8) + (H/16 * W/16) + (H/32 * W/32), 1]

        outputs[..., :2] = (outputs[..., :2] + grids) * strides
        outputs[..., 2:4] = torch.exp(outputs[..., 2:4]) * strides
        return outputs      # outputs: [1, 23625, 6], 23625 = 1440/32 * 800/32 * (1 + 2^2 + 2^4)
    ...
    def get_losses(
        ...
        if mixup:
            label_cut = labels[..., :5]     # filter out track_id in labels
        else:
            label_cut = labels
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                id_target = outputs.new_zeros((0))                          # ReID. [matched_anchor]
                ...
                gt_ids = track_ids[batch_idx, :num_gt]                      # ReID target, [matched_anchor]
                ...
                try:
                    (
                        ...
                        gt_matched_ids,                     # ReID. [matched_anchor], id of matched anchors
            ...
            id_targets.append(id_target)                # ReID. id target
        ...
        id_targets = torch.cat(id_targets, 0)       # ReID. [matched_anchor]
        id_targets = id_targets.to(torch.int64)     # ReID. [matched_anchor], float16 to int
        ...
        # loss = reg_weight * loss_iou + loss_obj + loss_cls + loss_l1      # original loss (only detection)
        # loss = reg_weight * loss_iou + loss_obj + loss_cls + loss_l1 + 0.5 * loss_id        # ReID. set weight of loss_id to 0.5
        # loss = loss_id          # only train id head

        # ReID. Uncertainty Loss
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, gt_ids, num_gt, fg_mask):
        ...
        # ---------------------------------------------------------------
        matching_matrix = torch.zeros_like(cost)        # [num_gt, matched_anchors]

        ious_in_boxes_matrix = pair_wise_ious           # [num_gt, matched_anchors]
        n_candidate_k = min(10, ious_in_boxes_matrix.size(1))   # int
        topk_ious, _ = torch.topk(ious_in_boxes_matrix, n_candidate_k, dim=1)       # [num_gt, n_candidate_k]
        dynamic_ks = torch.clamp(topk_ious.sum(1).int(), min=1)                 # [num_gt], value k of dynamic k
        for gt_idx in range(num_gt):        # iteration over [num_gt]
            _, pos_idx = torch.topk(
                cost[gt_idx], k=dynamic_ks[gt_idx].item(), largest=False        # index of positives of current gt
            )
            matching_matrix[gt_idx][pos_idx] = 1.0                              # set to 1 in corresponding index

        del topk_ious, dynamic_ks, pos_idx

        anchor_matching_gt = matching_matrix.sum(0)         # [matched_anchor]
        ...
        fg_mask[fg_mask.clone()] = fg_mask_inboxes      # update fg_mask. fg_mask: [all_anchors], fg_mask_inboxes: [matched_anchor]

        matched_gt_inds = matching_matrix[:, fg_mask_inboxes].argmax(0)     # [k_anchors]
        gt_matched_classes = gt_classes[matched_gt_inds]        # gt classes for [k_anchors]
        gt_matched_ids = gt_ids[matched_gt_inds]                # ReID, gt ids for matched anchors
